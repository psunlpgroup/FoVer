...
def get_verification_from_hidden_states_of_causal_model(
    ...
    elif model.config.architectures[0] in ["LlamaForCausalLM", "Qwen2ForCausalLM", "Qwen3ForCausalLM"]:
    ...
    for output in raw_verification_outputs:
        features = torch.tensor(output["response"], dtype=torch.bfloat16, device=model.device)
    ...
    for verification_score_type in ["logprob_min"]:
        ...
        for sample_idx in range(args.sample_k):
            ...
            verification_outputs_path = get_verification_for_sample_and_rank_outputs_path(
                ...
                few_shot_verification=args.few_shot_verification,  # few-shot is not implemented yet
            )
            ...
            print(
                f"Saved processed verification scores for "
                f"sample idx {sample_idx} to {verification_scores_path_for_this_idx}"
            )
