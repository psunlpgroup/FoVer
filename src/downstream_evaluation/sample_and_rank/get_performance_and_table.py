...
from src.config import \
    ...
    get_downstream_evaluation_datasets_list, \
...
class SampleAndRankPerformanceAndTableTap(Tap):
    ...
    # "few-shot"
    verification_prompt_type: PROMPT_TYPE = "multi-turn"
    # "multi-turn" is for the new prompt
    sample_k: int = 5
    # number of samples to generate for each prompt
    selection_method: str = "max"
    # "max" or "weighted_majority"
    few_shot_verification: bool = False
    # whether to use few-shot verification or not
    add_bootstrap_test: bool = False
...
def get_sample_and_rank_selected_output_path_dict(
    ...
    if args.verification_prompt_type == "zero-shot":
        ...
        selected_train_dataset_names_list = []  # to be updated
    else:
        raise NotImplementedError(
            f"Verification prompt type {args.verification_prompt_type} is not " \
            "supported."
        )
...
def main():
    ...
    all_evaluation_dataset_names_list = \
        get_downstream_evaluation_datasets_list(args.evaluation_mode)
    
    # create sub dataset list for eacy category in downstream_evaluation_dataset_name_to_category_dict
    downstream_evaluation_datasets_by_category: dict[str, list[str]] = {}
    for dataset_name in all_evaluation_dataset_names_list:
        category = downstream_evaluation_dataset_name_to_category_dict[dataset_name]
        if category not in downstream_evaluation_datasets_by_category:
            downstream_evaluation_datasets_by_category[category] = []
        downstream_evaluation_datasets_by_category[category].append(dataset_name)
    
    downstream_evaluation_datasets_by_category["analysis"] = analysis_dataset_list
    ...
    for base_model_name, initial_response_model_name in \
            [
                ...
                (None, "llama_factory_finetuned_models/SFT_on_correct_reasoning_merged-Llama-3.1-8B-Instruct-Qwen2.5-7B-Instruct_5.0e-6"),
                # ("Qwen/Qwen3-8B", "Qwen/Qwen3-32B"),
                # ("meta-llama/Llama-3.1-8B-Instruct", "Qwen/Qwen3-32B"),
                # ("Qwen/Qwen2.5-14B-Instruct", "Qwen/Qwen2.5-14B-Instruct"),
                # ("meta-llama/Llama-3.1-8B-Instruct", "google/gemma-3-27b-it"),
                # ("Qwen/Qwen2.5-7B-Instruct", "google/gemma-3-27b-it")
            ]:
        for verification_score_type in ["logprob_min"]:  # "logprob_min", "num_incorrect_steps"
            
            for category, category_evaluation_dataset_names_list in \
                    downstream_evaluation_datasets_by_category.items():
            
                # get evaluation metrics
                evaluation_metrics_dict: dict[str, dict] = {}
                evaluation_metrics_path_dict: dict[str, dict] = {}  # for logging
                for evaluation_dataset_name in category_evaluation_dataset_names_list:
                    ...
                    if base_model_name is not None:
                        ...
                        # only get result for base model
                        outputs_path_dict = {
                            "base_model": get_downstream_evaluation_initial_responses_path(
                                dataset_name=evaluation_dataset_name,
                                model_name=initial_response_model_name,
                ...
                if base_model_name is None:
                    pass
                elif args.evaluation_mode == "final_evaluation" and args.add_bootstrap_test:
                ...
                # save directory
                full_verification_prompt_name = \
                    f"{args.verification_prompt_type}_few-shot" if args.few_shot_verification else \
                ...
                sample_and_rank_table_dir = \
                    downstream_evaluation_tables_dir / "sample_and_rank" \
                        ...
                        / f"verification_model={base_model_name.split('/')[-1] if base_model_name is not None else "none"}" \
                        / full_verification_prompt_name / verification_score_type \
                        / f"selection_method={args.selection_method}" / f"sample_k={args.sample_k}" \
                        / category
                sample_and_rank_table_dir.mkdir(exist_ok=True, parents=True)


                if args.evaluation_mode == "final_evaluation" and args.add_bootstrap_test:
                ...
                table = []
                first_row = [f"{'Verifier':190s}"] \
                    + [f"{dname:18s}" for dname in
                        get_downstream_evaluation_datasets_display_name_list(
                            category_evaluation_dataset_names_list
                        )
                    ] \
                    + ["Average"]
                ...
                for metric_name in ["accuracy"]:
                    for method_name in list(evaluation_metrics_dict.values())[0].keys():
                        ...
                        row = [f"{method_display_name:190s}"]
