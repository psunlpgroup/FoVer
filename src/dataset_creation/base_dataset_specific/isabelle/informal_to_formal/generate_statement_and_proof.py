...
from src.dataset_creation.base_dataset_specific.isabelle.\
    informal_to_formal.get_few_shot_prompt import \
        ...
        get_few_shot_prompt_for_formal_proof_generation
...
class IsabelleInformalToFormalTap(Tap):
    ...
    conversion_model_name: str = "meta-llama/Llama-3.3-70B-Instruct"
    formal_proof_generation_model_name: str = "Qwen/Qwen3-32B"
...
def get_prompt_for_formal_proof_generation_path(
        dataset_name: str,
        initial_generation_model_name: str, conversion_model_name: str,
        formal_proof_model_name: str,
        split: str, seed: str = "selected") -> Path:
    
    initial_short_name = initial_generation_model_name.split("/")[-1]
    conversion_short_name = conversion_model_name.split("/")[-1]
    formal_proof_short_name = formal_proof_model_name.split("/")[-1]
    return model_inputs_dir / "isabelle" / "formal_proof_generation" / \
        dataset_name / \
        f"initial_generation={initial_short_name}" / \
        f"conversion={conversion_short_name}" / \
        f"formal_proof_model={formal_proof_short_name}" / \
        f"seed={seed}" / f"{split}.jsonl"
...
def get_converted_formal_statement_or_proofs_path(
    ...
    if statement_or_proof == "proof":
        # the new version does not use "proof" option here
        raise ValueError(
            "For generated formal proof, use "
            "`get_generated_formal_proof_path` instead."
        )
...
def get_generated_formal_proof_path(
        dataset_name: str,
        initial_generation_model_name: str, conversion_model_name: str,
        formal_proof_model_name: str,
        split: str) -> Path:
    ...
    conversion_short_name = conversion_model_name.split("/")[-1]
    formal_proof_short_name = formal_proof_model_name.split("/")[-1]
    return model_responses_dir / "isabelle" / "formal_proof_generation" / \
        dataset_name / \
        f"initial_generation={initial_short_name}" / \
        f"conversion={conversion_short_name}" / \
        f"formal_proof_model={formal_proof_short_name}" / f"{split}.jsonl"


def get_generated_formal_proofs_thy_file_path(
        dataset_name: str,
        initial_generation_model_name: str, conversion_model_name: str,
        formal_proof_model_name: str,
        split: str, data_id: str) -> Path:
    """ Get the path to the thy file of the generated formal proofs for the
    given dataset, model, and split. """
...
def main():
    ...
    for split in splits_list:
        # 1. Generate formal statement by converting informal statement
        # 2. Generate formal proof directly by args.formal_proof_model_name
        
        # This process is motivated by one proposed in the dtv paper.
        # However, in dtv paper, the proof generation is also done by converting
        # informal solution to formal proof.
        # Here, we directly generate formal proof using a strong model.
        ...
        ###
        # 2. Generate formal proof directly from args.formal_proof_model_name
        ...
        for idx, d in enumerate(informal_data):
            ...
            prompt = get_few_shot_prompt_for_formal_proof_generation(
                dataset_name=args.dataset_name,
                model_name=args.formal_proof_generation_model_name,
        ...
        # save prompts
        proof_prompt_path = \
            get_prompt_for_formal_proof_generation_path(
                dataset_name=args.dataset_name,
                initial_generation_model_name=args.base_model_name,
                conversion_model_name=args.conversion_model_name,
        ...
        for idx, d in enumerate(formal_data):
            ...
            formal_proof_thy_path = get_generated_formal_proofs_thy_file_path(
                dataset_name=args.dataset_name,
                initial_generation_model_name=args.base_model_name,
                conversion_model_name=args.conversion_model_name,
                formal_proof_model_name=args.formal_proof_generation_model_name,
